...
// ListUpgradeParts renvoie une page du catalogue d'un jeu + le total (count fenêtré).
...
SELECT id, game, category, name, level, pi_delta, weight_delta_kg,
       power_delta_hp, torque_delta_nm, source, last_verified,
       count(*) OVER() AS total
FROM upgrade_parts
WHERE game = $1
...
LIMIT $3 OFFSET $4`
	...
	for rows.Next() {
		...
		if err := rows.Scan(&p.ID, &p.Game, &p.Category, &p.Name, &p.Level,
			...
			&p.Source, &p.LastVerified, &total); err != nil {
...
// Joint car_upgrades → upgrade_parts. Voiture inconnue ou sans upgrade → page vide.
...
SELECT p.id, p.game, p.category, p.name, p.level, p.pi_delta, p.weight_delta_kg,
       ...
       cu.requires_part_id, cu.exclusive_group, count(*) OVER() AS total
...
LIMIT $3 OFFSET $4`
	...
	for rows.Next() {
		...
		if err := rows.Scan(&u.Part.ID, &u.Part.Game, &u.Part.Category, &u.Part.Name,
			...
			&u.RequiresPartID, &u.ExclusiveGroup, &total); err != nil {
